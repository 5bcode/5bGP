...
public class FlipFinderPanel extends PluginPanel {
	...
	private static final String CONFIG_GROUP = "flipto5b";
	...
	private static final Color COLOR_PRICE_HIGHER_BG = new Color(60, 90, 60); // Subtle green - selling higher than
																				// estimate
	...
	// High volume items (>500k daily trades): switch to loss-minimizing after 10
	// min
	...
	private final transient FlipTo5BConfig config;
	...
	private final JComboBox<FlipTo5BConfig.FlipStyle> flipStyleDropdown;
	...
	private final transient FlipTo5BPlugin plugin; // Reference to plugin to store recommended prices
	...
	public FlipFinderPanel(FlipTo5BConfig config, FlipSmartApiClient apiClient, ItemManager itemManager,
			FlipTo5BPlugin plugin, ConfigManager configManager) {
		super(false);
		this.config = config;
		...
		flipStyleDropdown = new JComboBox<>(FlipTo5BConfig.FlipStyle.values());
		...
		flipStyleDropdown.addActionListener(e -> {
			...
			FlipTo5BConfig.FlipStyle selectedStyle = (FlipTo5BConfig.FlipStyle) flipStyleDropdown.getSelectedItem();
	...
	private void buildMainPanel() {
		...
		flipStyleDropdown.setRenderer(new DefaultListCellRenderer() {
			...
			public Component getListCellRendererComponent(JList<?> list, Object value, int index,
				...
				if (c instanceof JLabel && value instanceof FlipTo5BConfig.FlipStyle) {
					FlipTo5BConfig.FlipStyle style = (FlipTo5BConfig.FlipStyle) value;
		...
		// Subscribe message for non-premium users (hidden by default until we know
		// premium status)
	...
	private void handleDiscordLogin() {
		...
		apiClient.startDeviceAuthAsync().thenAccept(response -> {
			...
			SwingUtilities.invokeLater(() -> showLoginStatus("Complete login in your browser...", true));
	...
	private void startDeviceAuthPolling(String deviceCode, int pollIntervalSeconds, int expiresInSeconds) {
		...
		deviceAuthPollTask = deviceAuthScheduler.scheduleAtFixedRate(() -> {
			// Check if the current device code has changed or polling was stopped
			// externally
			if (currentDeviceCode == null || !currentDeviceCode.equals(deviceCode)) {
				// Polling cancelled or restarted for a different code
				stopDeviceAuthPolling();
				return;
			}
			...
			apiClient.pollDeviceStatusAsync(deviceCode).thenAccept(status -> {
				...
				switch (status.getStatus()) {
					case "authorized":
						...
						SwingUtilities.invokeLater(() -> onAuthenticationSuccess("Login successful!", true));
						break;
	...
	/**
	 * Handle successful authentication - notify plugin and transition to main
	 * panel.
	 * 
	 ...
	 * Refresh flip recommendations, active flips, and completed flips.
	 *
	 * @param shuffleSuggestions if true, randomizes suggestions within quality
	 *                           tiers (for manual refresh)
	 */
	public void refresh(boolean shuffleSuggestions) {
		...
		// This prevents the focused item from disappearing while user is
		// mid-transaction
	...
	private void refreshRecommendations(boolean shuffleSuggestions) {
		...
		// Don't clear container yet - keep showing old recommendations until new data
		// arrives
		...
		FlipTo5BConfig.FlipStyle selectedStyle = (FlipTo5BConfig.FlipStyle) flipStyleDropdown.getSelectedItem();
		String flipStyle = selectedStyle != null ? selectedStyle.getApiValue()
				: FlipTo5BConfig.FlipStyle.BALANCED.getApiValue();
		...
		apiClient.getFlipRecommendationsAsync(cashStack, flipStyle, limit, randomSeed).thenAccept(response -> {
			SwingUtilities.invokeLater(() -> {
				...
				// Update premium status from flip-finder response and show/hide subscribe
				// message
		...
		apiClient.getActiveFlipsAsync(rsn).thenAccept(response -> {
			SwingUtilities.invokeLater(() -> {
				...
				if (response.getActiveFlips() != null) {
					...
					// Note: Using getActiveFlipItemIds() instead of WithInventory() to avoid thread
					// issues
				...
				java.util.List<FlipTo5BPlugin.PendingOrder> pendingOrders = plugin.getPendingBuyOrders();
	...
	public void updatePendingOrders(java.util.List<FlipTo5BPlugin.PendingOrder> pendingOrders) {
	 ...
	 */
	private void displayActiveFlipsAndPending(java.util.List<ActiveFlip> activeFlips,
			java.util.List<FlipTo5BPlugin.PendingOrder> pendingOrders) {
		activeFlipsListContainer.removeAll();

		// Build a map of pending orders by itemId for smart deduplication
		java.util.Map<Integer, java.util.List<FlipTo5BPlugin.PendingOrder>> pendingByItemId = buildPendingOrdersMap(
				pendingOrders);

		// First show pending orders (items currently in GE buy slots)
		for (FlipTo5BPlugin.PendingOrder pending : pendingOrders) {
	 ...
	 */
	private java.util.Map<Integer, java.util.List<FlipTo5BPlugin.PendingOrder>> buildPendingOrdersMap(
			java.util.List<FlipTo5BPlugin.PendingOrder> pendingOrders) {
		java.util.Map<Integer, java.util.List<FlipTo5BPlugin.PendingOrder>> pendingByItemId = new java.util.HashMap<>();
		for (FlipTo5BPlugin.PendingOrder pending : pendingOrders) {
	...
	/**
	 * Determine if an active flip should be shown (not duplicated by pending
	 * orders)
	 * 
	 ...
	 * Active flips should only show for COLLECTED items (no longer in GE, waiting
	 * to sell).
	 */
	private boolean shouldShowActiveFlip(ActiveFlip flip,
			java.util.Map<Integer, java.util.List<FlipTo5BPlugin.PendingOrder>> pendingByItemId) {
		java.util.List<FlipTo5BPlugin.PendingOrder> matchingPending = pendingByItemId.get(flip.getItemId());
		...
		// Skip the active flip to avoid duplicates - the pending order panel shows the
		// current state.
	 ...
	 * This saves API requests and battery by not polling while the player can't use
	 * the GE anyway.
	 */
	...
	private HeaderPanels createItemHeaderPanels(int itemId, String itemName, Color bgColor) {
		...
		// Panel is ~220px wide, minus item icon (36px), chart icon (20px), padding
		// (14px) = ~150px for name
	...
	/**
	 * Create a clickable chart icon label that opens the item's page on the
	 * website.
	...
	private JLabel createChartIconLabel(int itemId) {
		...
		java.awt.image.BufferedImage chartIcon = new java.awt.image.BufferedImage(14, 14,
		...
		chartLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				...
				java.awt.image.BufferedImage hoverIcon = new java.awt.image.BufferedImage(14, 14,
						java.awt.image.BufferedImage.TYPE_INT_ARGB);
	...
	private String formatMarginText(int marginPerItem, double roi, boolean isLoss) {
		...
		// Handle Infinity/NaN ROI (happens when cost is 0, e.g. pending orders with no
		// fills)
	...
	/**
	 * Apply a price indicator background color to the active flip panel and its
	 * children.
	 * Used to visually indicate whether the sell price is higher or lower than the
	 * original recommendation.
	 */
	private void applyPriceIndicatorBackground(JPanel panel, JPanel topPanel, JPanel namePanel, JPanel detailsPanel,
			Color bgColor) {
		panel.setBackground(bgColor);
		topPanel.setBackground(bgColor);
		namePanel.setBackground(bgColor);
	...
	/**
	 * Get the base background color for a panel (either price indicator color or
	 * default).
	...
	public void validateFocus() {
		...
		if (!hasFlipForItem(currentFocusedItemId)) {
			log.debug("Clearing Flip Assist focus - item {} no longer in recommendations or active flips",
	...
	private void setFocus(ActiveFlip flip, JPanel panel) {
		...
		if (cachedSellPrice != null && cachedSellPrice > 0) {
			...
			apiClient.getItemAnalysisAsync(flip.getItemId()).thenAccept(analysis -> {
				...
				int sellPrice = smartSellPrice != null ? smartSellPrice
						: calculateMinProfitableSellPrice(flip.getAverageBuyPrice());
	 ...
	 */
	private void setFocus(FlipTo5BPlugin.PendingOrder pending, JPanel panel) {
	 ...
	 * This is the "smart" price that considers time thresholds and market
	 * conditions.
	 * 
	...
	/**
	 * Set focus from external source (e.g., auto-focus when setting up a sell
	 * offer).
	...
	private JPanel createActiveFlipPanel(ActiveFlip flip) {
			...
			SwingUtilities.invokeLater(() -> {
				...
				if (smartSellPrice != null && smartSellPrice > 0) {
					...
					if (recommendedPrice != null && recommendedPrice > 0 && !smartSellPrice.equals(recommendedPrice)) {
						...
						// Apply to panel and all child panels (not focused panel - that has its own
						// style)
					...
					if (pastThreshold && totalProfit < 0) {
						panel.setToolTipText(
								"Price adjusted to minimize loss. Original recommended price was not achievable.");
					}
				} else {
		...
		// Store default background color as client property (will be overwritten if
		// price indicator is applied)
	 ...
	 */
	private JPanel createPendingOrderPanel(FlipTo5BPlugin.PendingOrder pending) {
	...
	private JPanel createCompletedFlipPanel(CompletedFlip flip) {
		...
		Color backgroundColor = flip.isSuccessful() ? new Color(40, 60, 40) : // Dark green for profit
		...
		// Details section with profit/loss info - use GridBagLayout for tighter column
		// spacing
		...
		Color profitColor = flip.isSuccessful() ? COLOR_PROFIT_GREEN : // Bright green
		...
		gbc.gridx = 0;
		gbc.gridy = 0;
		detailsPanel.add(qtyLabel, gbc);
		gbc.gridx = 1;
		gbc.gridy = 0;
		gbc.insets = new Insets(1, 0, 1, 0);
		detailsPanel.add(buyPriceLabel, gbc);
		gbc.gridx = 0;
		gbc.gridy = 1;
		gbc.insets = new Insets(1, 0, 1, 15);
		detailsPanel.add(investedLabel, gbc);
		gbc.gridx = 1;
		gbc.gridy = 1;
		gbc.insets = new Insets(1, 0, 1, 0);
		detailsPanel.add(sellPriceLabel, gbc);
		gbc.gridx = 0;
		gbc.gridy = 2;
		gbc.insets = new Insets(1, 0, 1, 15);
		detailsPanel.add(profitLabel, gbc);
		gbc.gridx = 1;
		gbc.gridy = 2;
		gbc.insets = new Insets(1, 0, 1, 0);
		detailsPanel.add(roiLabel, gbc);
		...
		panel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (!expanded) {
					...
					String duration = hours > 0 ? String.format("%dh %dm", hours, minutes)
	...
	private void dismissActiveFlip(ActiveFlip flip) {
		int result = JOptionPane.showConfirmDialog(
				this,
				String.format(
						"Remove %s from active flips?%n%nThis will hide it from tracking.%nUse this if you sold/used the items outside of the GE.",
						flip.getItemName()),
				"Dismiss Active Flip",
	 ...
	 * @return The recommended sell price, or null to indicate need to fetch market
	 *         price
	 */
