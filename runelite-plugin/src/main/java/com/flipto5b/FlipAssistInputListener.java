...
public class FlipAssistInputListener implements KeyListener {
	...
	private static final String CHAT_MESSAGE_PREFIX = "<col=00e5ff>[FlipTo5B]</col> ";
	...
	@Inject
	public FlipAssistInputListener(Client client, ClientThread clientThread, FlipTo5BConfig config,
	...
	// Track the keyPressed event we're handling to consume its corresponding
	// keyTyped
	...
	private void handleFlipAssistAction(FocusedFlip focusedFlip) {
		...
		if (isLikelyPriceInput()) {
			...
			sendChatMessage(focusedFlip.getItemName() + " price set to "
					+ String.format("%,d", focusedFlip.getCurrentStepPrice()) + " gp");
		} else {
			setInputValue(focusedFlip.getCurrentStepQuantity());
			sendChatMessage(focusedFlip.getItemName() + " quantity set to "
					+ String.format("%,d", focusedFlip.getCurrentStepQuantity()));
		}
	}
	 ...
	 * If quantity is already set to a non-default value, we're likely setting
	 * price.
	 * 
	...
	private boolean determineFromGEState() {
		...
		// If quantity has been set (> 1, since default is often 1) and we're in an
		// input,
	 ...
	 * Returns Optional containing true if price, false if quantity, empty if no
	 * match.
	 */
	...
	private boolean isSearchTextMatchingFocusedItem(FocusedFlip focusedFlip) {
		...
		// This allows the hotkey to work when GE_LAST_SEARCHED auto-populates the
		// search
