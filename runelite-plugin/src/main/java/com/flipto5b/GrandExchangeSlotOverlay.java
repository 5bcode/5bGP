...
public class GrandExchangeSlotOverlay extends Overlay {
	...
	@Inject
	private GrandExchangeSlotOverlay(Client client, FlipTo5BConfig config, FlipTo5BPlugin plugin) {
		this.client = client;
		this.config = config;
		this.plugin = plugin;
	...
	private static class TooltipData {
		...
		FlipTo5BPlugin.WikiPrice wikiPrice;
		int offerPrice;
		int x;
		int y;
	}
	...
	private TooltipData renderSlot(Graphics2D graphics, int slot, GrandExchangeOffer offer) {
		...
		FlipTo5BPlugin.TrackedOffer trackedOffer = plugin.getTrackedOffer(slot);
		FlipTo5BPlugin.OfferCompetitiveness competitiveness = plugin.calculateCompetitiveness(trackedOffer);
	 ...
	 */
	private void renderSlotBorder(Graphics2D graphics, Rectangle bounds,
			FlipTo5BPlugin.OfferCompetitiveness competitiveness) {
	 ...
	 */
	private void renderSlotTimer(Graphics2D graphics, Rectangle bounds, FlipTo5BPlugin.TrackedOffer trackedOffer) {
	 ...
	 */
	private void drawSlotBorder(Graphics2D graphics, Rectangle bounds,
			FlipTo5BPlugin.OfferCompetitiveness competitiveness) {
		Color borderColor;
	...
	/**
	 * Draw completion checkbox indicator (green checkmark for orders ready to
	 * collect)
	 */
	...
	private void drawPriceTooltip(Graphics2D graphics, int x, int y, GrandExchangeOffer offer,
			FlipTo5BPlugin.WikiPrice wikiPrice, int offerPrice) {
	 ...
	 */
	private java.util.List<String> buildTooltipLines(FlipTo5BPlugin.WikiPrice wikiPrice, int offerPrice) {
		java.util.List<String> lines = new java.util.ArrayList<>();

		if (wikiPrice != null && (wikiPrice.high > 0 || wikiPrice.low > 0)) {
			if (wikiPrice.high > 0) {
				lines.add("Insta Buy: " + NUMBER_FORMAT.format(wikiPrice.high) + " gp");
			}
			if (wikiPrice.low > 0) {
				lines.add("Insta Sell: " + NUMBER_FORMAT.format(wikiPrice.low) + " gp");
			}
		} else {
	 ...
	 */
	private Color determineYourPriceColor(FlipTo5BPlugin.WikiPrice wikiPrice, int offerPrice, boolean isBuy) {
		if (wikiPrice == null || (wikiPrice.high <= 0 && wikiPrice.low <= 0)) {
			return Color.WHITE;
		}

		boolean isCompetitive = isBuy
				? offerPrice >= wikiPrice.low
				: offerPrice <= wikiPrice.high;
	 ...
	 */
	private void drawTooltipBackground(Graphics2D graphics, int x, int y, java.util.List<String> lines,
			FontMetrics fm) {
	...
	/**
	 * Draw orange glow highlights on GE buttons based on the current flip assist
	 * step.
