...
public class FlipSmartApiClient {
	...
	@Inject
	public FlipSmartApiClient(FlipTo5BConfig config, Gson gson, OkHttpClient okHttpClient) {
		this.config = config;
	 ...
	 * @param responseHandler Function to process successful response body and
	 *                        return result
	 ...
	 * 
	 * @return CompletableFuture with AuthResult containing success status and
	 *         message
	 */
	 ...
	 * Returns a device code and verification URL that the user should open in their
	 * browser.
	 * 
	 * @return CompletableFuture with DeviceAuthResponse containing device_code and
	 *         verification_url
	 */
	...
	/**
	 * Poll the device authorization status to check if the user has completed
	 * Discord OAuth.
	 * 
	 ...
	 */
	public CompletableFuture<FlipFinderResponse> getFlipRecommendationsAsync(Integer cashStack, String flipStyle,
			int limit, Integer randomSeed) {
		String apiUrl = getApiUrl();
		...
		return executeAuthenticatedAsync(requestBuilder, jsonData -> gson.fromJson(jsonData, FlipFinderResponse.class));
	}
	...
	public static class TransactionRequest {
		...
		public static class Builder {
			...
			public Builder geSlot(Integer geSlot) {
				this.geSlot = geSlot;
				return this;
			}

			public Builder recommendedSellPrice(Integer price) {
				this.recommendedSellPrice = price;
				return this;
			}

			public Builder rsn(String rsn) {
				this.rsn = rsn;
				return this;
			}

			public Builder totalQuantity(Integer qty) {
	...
	public CompletableFuture<ActiveFlipsResponse> getActiveFlipsAsync(String rsn) {
		...
		return executeAuthenticatedAsync(requestBuilder,
				jsonData -> gson.fromJson(jsonData, ActiveFlipsResponse.class));
	}
	...
	/**
	 * Sync the filled quantity for an active flip when the plugin detects a
	 * mismatch.
	...
	public CompletableFuture<Boolean> syncActiveFlipAsync(int itemId, String itemName, int filledQuantity,
		...
		return executeAuthenticatedAsync(requestBuilder, jsonData -> {
			JsonObject responseObj = gson.fromJson(jsonData, JsonObject.class);
			int previousQty = responseObj.has("previous_quantity") ? responseObj.get("previous_quantity").getAsInt()
					: 0;
	...
	public CompletableFuture<CompletedFlipsResponse> getCompletedFlipsAsync(int limit, String rsn) {
		...
		return executeAuthenticatedAsync(requestBuilder,
				jsonData -> gson.fromJson(jsonData, CompletedFlipsResponse.class));
	}
	...
	public void getDumpsAsync(String sortBy, int minProfit, int limit,
		...
		ensureAuthenticatedAsync().thenAccept(authSuccess -> {
			...
			// Build query parameters
			HttpUrl baseUrl = HttpUrl.parse(getApiUrl() + "/dumps");
			if (baseUrl == null) {
				if (onError != null)
					onError.accept("Invalid API URL");
				return;
			}
	...
	/**
	 * Fetch market dumps with default parameters (recency sort, no min profit,
	 * limit 50)
	 */
	...
	public CompletableFuture<BankSnapshotStatusResponse> checkBankSnapshotStatusAsync(String rsn) {
		...
		return executeAuthenticatedAsync(requestBuilder,
				jsonData -> gson.fromJson(jsonData, BankSnapshotStatusResponse.class));
	}
	...
	public CompletableFuture<BankSnapshotResponse> createBankSnapshotAsync(String rsn, java.util.List<BankItem> items) {
		...
		return executeAuthenticatedAsync(requestBuilder,
				jsonData -> gson.fromJson(jsonData, BankSnapshotResponse.class));
	}
